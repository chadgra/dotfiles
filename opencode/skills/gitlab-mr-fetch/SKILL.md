...
description: Fetch all context needed to review a GitLab MR: metadata, diff, base/head SHAs, existing inline comments, and changed file list. Use this skill when starting a GitLab MR review or when you need to understand what changed in an MR. Works on gitlab.com. Requires GITLAB_TOKEN env var.
---
...
if [ -z "$GITLAB_TOKEN" ]; then
  echo "No GitLab token found. Set \$GITLAB_TOKEN in your environment."
  exit 1
fi
...
## ⚠️ Token identity check

After fetching the MR, verify that the token resolves to a real user (not a project bot):

```bash
TOKEN_USER=$(curl --silent \
  --header "PRIVATE-TOKEN: ${GITLAB_TOKEN}" \
  "https://gitlab.com/api/v4/user" \
  | python3 -c "import json,sys; u=json.load(sys.stdin); print(u.get('username','unknown'), '| bot:', u.get('bot', False))")
echo "Posting comments as: $TOKEN_USER"
```

If the output shows `bot: True`, stop and warn the user:

> "⚠️ Your GITLAB_TOKEN is a project access token (bot), not a personal access token. Comments will be posted under the bot's name, not yours. To fix this: go to https://gitlab.com/-/user_settings/personal_access_tokens, create a token with `api` scope, and set it as GITLAB_TOKEN."

Ask the user if they want to continue anyway or stop to fix the token first.
